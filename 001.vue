<template>
  <div>{{ Math.round(RotateAngle) }}</div>
  <input type="range" min="0" max="360" v-model="RotateAngle">
  <!-- <input type="text" v-model="rects[selectedRectIndex].rotateAngle" /> -->
  <div class="draw-board">
    <svg style="background-color: white;" ref="svgRef" @mousedown="handleMouseDown(null, $event)"
      ...
      <rect
        ...
        v-for="(rect, index) in rects" :key="index" :x="rect.x" :y="rect.y" :width="rect.width" :height="rect.height"
        :fill="rect.color" />
      ...
      <g :transform="'rotate(' + RotateAngle + ' ' + getNowCentor.x + ' ' + getNowCentor.y + ')'">
        ...
        <rect :x="selectedRectX" :y="selectedRectY" :width="selectedRectWidth" :height="selectedRectHeight" fill="none"
          stroke="blue" />
        <!-- 添加缩放手柄 -->
        <rect v-if="selectedRectIndex !== -1" :x="selectedRectX - handleSize / 2" :y="selectedRectY - handleSize / 2"
          :width="handleSize" :height="handleSize" fill="white" stroke="black"
          @mousedown="handleMouseDown('tl', $event)" />
        <rect v-if="selectedRectIndex !== -1" :x="selectedRectX + selectedRectWidth - handleSize / 2"
          :y="selectedRectY - handleSize / 2" :width="handleSize" :height="handleSize" fill="white" stroke="black"
          @mousedown="handleMouseDown('tr', $event)" />
        <rect v-if="selectedRectIndex !== -1" :x="selectedRectX - handleSize / 2"
          :y="selectedRectY + selectedRectHeight - handleSize / 2" :width="handleSize" :height="handleSize" fill="white"
          stroke="black" @mousedown="handleMouseDown('bl', $event)" />
        <rect v-if="selectedRectIndex !== -1" :x="selectedRectX + selectedRectWidth - handleSize / 2"
          :y="selectedRectY + selectedRectHeight - handleSize / 2" :width="handleSize" :height="handleSize" fill="white"
          stroke="black" @mousedown="handleMouseDown('br', $event)" />
        ...
        <circle v-if="selectedRectIndex !== -1" class="handle-rotate-circle" :cx="selectedRectX + selectedRectWidth / 2"
          ...
          @mousedown="handleMouseDown('rotate', $event)" />
      </g>
...
export default defineComponent({
  ...
  computed: {
    ...
    overlapLine() {
      if (this.selectedRectIndex !== -1) {
        const selectedRect = this.rects[this.selectedRectIndex];

        for (let i = 0; i < this.rects.length; i++) {
          if (i !== this.selectedRectIndex) {
            const rect = this.rects[i];
            const maskedX = Math.max(rect.x, selectedRect.x);
            const maskedY = Math.max(rect.y, selectedRect.y);
            const maskedWidth = Math.min(rect.x + rect.width, selectedRect.x + selectedRect.width) - maskedX;
            const maskedHeight = Math.min(rect.y + rect.height, selectedRect.y + selectedRect.height) - maskedY;

            if (maskedWidth > 0 && maskedHeight > 0) {
              return {
                x1: maskedX + maskedWidth / 2,
                y1: maskedY + maskedHeight / 2,
